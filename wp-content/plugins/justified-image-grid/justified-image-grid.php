...
text-align:left;",
					...
					if(!empty($provider) && !empty($content_id)){
						...
						switch ($provider) {
							...
							case 'FB':
								...
								$photos_url = 'https://graph.facebook.com/v2.1/'.$content_id.'?fields=name,images&access_token='.$token;
		...
		function head_process($buffer){
			// Thinks to remove due to rewriting them
		...
		function head_end(){
			ob_end_flush();
			if(!empty($this->custom_og_image)){
			       echo $this->custom_og_image;

			}
			if(!empty($this->custom_og_title)){
			       echo $this->custom_og_title;
			}
			if(!empty($this->custom_og_description)){
			       echo $this->custom_og_description;
			}
...
\t<link rel='canonical' href='".$og_url_meta_for_fb."' />";
		...
		function jig_jquery_override(){
			...
			if($this->settings['conditional_script_loading'] == 'no'){
				...
				if($this->settings['load_bundled_lightbox'] == 'yes'){
					...
					wp_register_script('colorbox', plugins_url('js/jquery.colorbox-min.js', __FILE__), 'jquery', '1.5.13', true);
					wp_enqueue_script('colorbox');
					wp_register_style('colorbox-style', plugins_url('css/colorbox'.$this->settings['colorbox_design'].'.css', __FILE__), false, '1.5.13');
		...
		function jig_build_settings_page(){
			...
			<script type="text/javascript">
					...
					jQuery(document).ready(function(jQuery) {
						...
						addMiniSection('og_title_field',"<?php _e('Facebook settings for Smart deeplinking', 'jig_td'); ?>");
	...
	<?php
		...
		function jig_init_options(){
			if (!empty($_POST['preset']) && check_admin_referer('jig_presets','jig_presets_nonce')){
				if($_POST['preset'] !== 'default'){
				...
				if(!empty($_POST['new_custom_preset'])){
					...
					if(!empty($settings_to_store)){
						...
						$jig_new_preset_notice = "<div class='updated'><p><strong>".sprintf(__('%s has been successfully added!', 'jig_td'),$preset_settings['preset_name'])."</strong></p></div>"; 
			...
			if(!empty($this->settings['save_to_preset'])){
				...
				if(!empty($settings_to_store)){
					...
					$jig_preset_update_notice = "<div class='updated'><p><strong>".sprintf(__('%s has been successfully updated!', 'jig_td'),$this->custom_presets[$custom_preset_id]['preset_name'])."</strong></p></div>";
				...
				array($this, 'jig_print_general_input'),
					...
					array(	'id' => 'link_target',
							...
							'inputs' => array(
								...
								'videoplayer' => __('Dedicated video player page in the lightbox', 'jig_td'),
			...
			add_settings_field(
				...
				array(	'id' => 'conditional_script_loading',
						...
						'inputs' => array(
							...
							'no' => __('No: Load all scripts, supports AJAX / dynamic loading / animated page loads without refresh.', 'jig_td')
						)
				)
			);
			// jQuery source
			...
			add_settings_field(
				...
				array($this, 'jig_print_general_input'),
					...
					array(	'id' => 'prettyphoto_deeplinking',
							'help' => __("The smart and advanced options use several server-side logics to make individual Facebook like and share possible. The regular deeplinking doesn't work efficitently. <strong>Smart:</strong> Keeps the URL short, sends the photo and its title/description to Facebook like/share. Uses content IDs when available, but is based on and falls back to advanced deeplinking. <strong>Advanced:</strong> Allows individual like with thumbnail on FB, works with random image order.", 'jig_td'),
							'inputs' => array(
			...
			add_settings_field(
				...
				array($this, 'jig_print_general_input'),
					...
					array(	'id' => 'og_title_field',
							'help' => __("Choose a WP field as title on Facebook when liking/sharing. These are used for individual images. Currently only prettyPhoto has smart deeplinking. This is a site wide setting and not available to change per gallery. Other image sources use Title and Description fields.", 'jig_td'),
							'inputs' => array(
								'title' => __('Title', 'jig_td'),
								'description' => __('Description', 'jig_td'),
								'caption' => __('Caption', 'jig_td'),
			...
			add_settings_field(
				...
				array($this, 'jig_print_general_input'),
					...
					array(	'id' => 'og_description_field',
							'help' => __('Choose a WP field as description on Facebook when liking/sharing. If there is no text for this og:description to show (e.g. Facebook / Instagram source) but the content for og:title is available but too long, it will be shown as og:description instead. ', 'jig_td'),
							'inputs' => array(
								'description' => __('Description', 'jig_td'),
								'title' => __('Title', 'jig_td'),
								'caption' => __('Caption', 'jig_td'),
		...
		function jig_print_fb_authed(){
			...
			$output .= '<div id="jigFbAuthOtherUserPanel">
							<ol>
								...
								<li>'.__('Send him/her this link', 'jig_td').':<br /><input type="text" id="jigFbOtherUserLink" value="" data-force="https://www.facebook.com/v2.1/dialog/oauth?client_id='.trim($this->settings['fb_app_id']).'&scope=user_photos,read_stream' . '&redirect_uri='.urlencode(plugins_url('fb-auth-other-user.php', __FILE__)).'" /></li>
		...
		*/
		...
		function jig_init_shortcode($atts){
			...
			extract(shortcode_atts(array(
				...
				'recents_link'			=> 'no',
				'recents_link_text'		=> __('Read more_','jig_td'),
			...
			switch($gallery_type){
												...
												if($ng_lightbox_gallery == 'yes'){ // If gallery should be displayed as a lightbox
													...
													if(!empty($lightbox_images)){
														...
														foreach ($lightbox_images as $lightbox_image) {
															...
															$shadow_gallery .= '<a href="'.$data['url'].'" rel="'.$shadow_rel.'" '.$shadow_class.' title="'.$title_fragment.'"><img src="data:image/gif;base64,R0lGODlhAQABAPABAP///wAAACH5BAEKAAAALAAAAAABAAEAAAICRAEAOw%3D%3D" alt="'.$alt_fragment.'" /></a>';
														}
														$shadow_gallery .= "</div>";
													}
												}
				...
				case 'facebook':
					if(isset($this->settings['fb_authed'][$facebook_id])){
						...
						// Multi album mode 
						if(strpos($facebook_album,',') !== false || $fb_album_exclude == 'yes' || $facebook_album == 'latestone') // Only one album selected BUT in exclude mode
						{ 
						...
						if($facebook_album == 'overview' || $facebook_album == 'overview_only_albums'){
							if($fb_lightbox_album == 'yes'){
								$albums_url = 'https://graph.facebook.com/v2.1/'.$facebook_id.'?fields=albums.limit('.$limit.').fields(id,cover_photo,link,count,name,description,type,photos.limit('.$limit.').fields(images,source,height,width,name))&access_token='.$token;
							}else{
								$albums_url = 'https://graph.facebook.com/v2.1/'.$facebook_id.'?fields=albums.limit('.$limit.').fields(id,cover_photo,link,count,name,description,type,photos.limit(1).fields(images))&access_token='.$token;
							}

							if(!empty($facebook_album_multi) && $facebook_album_multi[0] == 'latestone'){
								$facebook_album = 'overview_only_albums';
								$fb_lightbox_album = 'yes';
								$albums_url = 'https://graph.facebook.com/v2.1/'.$facebook_id.'?fields=albums.limit(5).fields(id,cover_photo,link,count,name,description,type,photos.limit('.$limit.').fields(images,source,height,width,name))&access_token='.$token;
							}
									...
									foreach ($albums as $key => $album) {
										...
										if(!empty($album->count) && !empty($album->link) && !empty($album->photos->data)){
											if($fb_lightbox_album == 'no' && $fb_actual_cover_photo == 'yes' && !empty($album->cover_photo)){
												...
												$fb_cover_photo_url = 'https://graph.facebook.com/v2.1/'.$album->cover_photo.'?fields=images&access_token='.$token;
											...
											if(!empty($album->photos)){
												...
												if($fb_lightbox_album == 'yes'){ // If gallery should be displayed as a lightbox
													...
													if($album->count !== count($subalbum->data)){ // Facebook returned less images (usually 100) than the limit
														$additional_photos = $this->facebook_api_call($album->photos->paging->next, $facebook_caching, $limit, count($subalbum->data));
													...
													foreach ($subalbum->data as $subalbum_image) {
														...
														$shadow_gallery .= '<a href="'.$shadow_data['url'].'" rel="'.$shadow_rel.'" '.$shadow_class.' title="'.$title_fragment.'"><img src="data:image/gif;base64,R0lGODlhAQABAPABAP///wAAACH5BAEKAAAALAAAAAABAAEAAAICRAEAOw%3D%3D" alt="'.$alt_fragment.'" /></a>';
						...
						if($facebook_album == 'feed'){
							...
							$feed_url = 'https://graph.facebook.com/v2.1/'.$facebook_id.'/feed?fields=picture,caption,description,message,object_id'.$limit_parameter.'&access_token='.$token;
							...
							if(!empty($photos->data)){
								...
								foreach ($photos->data as &$feed_post) {
									if(!(isset($feed_post->picture) && isset($feed_post->object_id))){
										...
										$feed_url = 'https://graph.facebook.com/v2.1/'.$feed_post->object_id.'?fields=images,source,height,width&access_token='.$token;
										if($facebook_overview_caching > 0){
										...
										if(isset($photo_object->images)){
											...
											if($download_link != 'no'){
												$data['download'] = esc_attr(stripslashes('<a href="'.plugins_url('download.php', __FILE__).'?file='.urlencode($photo_object->images[0]->source).'">'.__($download_link_text,'jig_td').'</a>'));
											}
						...
						if($facebook_album !== 'overview' && $facebook_album !== 'overview_only_albums' && $facebook_album !== 'feed'){
							$photos_url = "https://graph.facebook.com/v2.1/".$facebook_album."/photos?fields=source,height,width,name".($facebook_image_size == 'normal' ? '' : ',images').$limit_parameter.'&access_token='.$token;
							...
							if(!empty($photos) && empty($photos['message'])){
								...
								foreach ($photos as $image) {
									...
									if($download_link != 'no'){
										$data['download'] = esc_attr(stripslashes('<a href="'.plugins_url('download.php', __FILE__).'?file='.urlencode($image->images[0]->source).'">'.__($download_link_text,'jig_td').'</a>'));
									}
								...
								if(($facebook_description == 'yes' || $facebook_description == 'above') && empty($facebook_description_displayed_already)){
									$facebook_album_url = "https://graph.facebook.com/v2.1/".$facebook_album."?fields=description".'&access_token='.$token;
									if($facebook_caching > 0){
						...
						if(count($rss_items) > 0){
							...
							foreach ($rss_items as $rss_item_index => &$item) {
								...
								if(!empty($enclosure_links)){
									...
									if($enclosure_link == '//?#'){ // Weirdest thing ever, but it fixes non-appearing youtube feeds
				...
				if($link_target == 'off' && !($gallery_type == 'facebook' && $image_element['link_target'] == 'video')){
					...
					if($image_element['link_target'] == 'videoplayer'){
						...
						if ($lightbox !== 'prettyphoto'
							...
							if($lightbox == 'photoswipe' || $lightbox == 'colorbox' || $lightbox == 'carousel' || $lightbox == 'socialgallery'){
						...
						if((strpos($image_element['link'], 'youtube.com/') !== false || strpos($image_element['link'], 'vimeo.com/') !== false)
							&&
							($lightbox == 'photoswipe' || $lightbox == 'colorbox' || $lightbox == 'carousel' || $lightbox == 'socialgallery')){
					...
					$lightbox_JS .= ".magnificPopup({
											...
											zoom: {
												...
												opener: function(openerElement) {
													if(openerElement.parent().hasClass('jig-hiddenGallery')){
		...
		function jig_fb_auth(){
			...
			if(empty($_REQUEST["code"])) {
				...
				$dialog_url = "https://www.facebook.com/v2.1/dialog/oauth?client_id=" 
			...
			if($jig_session['state'] && ($jig_session['state'] === $_REQUEST['state'])) {
				...
				$graph_url = "https://graph.facebook.com/v2.1/me?fields=name,picture&access_token=" 
				. $params['access_token'];
			    ...
			    $albums_url = "https://graph.facebook.com/v2.1/me/albums?fields=id,link,count,from&limit=1000&access_token=".$params['access_token'];
		...
		function jig_add_fb_page($token = '', $user_name = ''){
			...
			if($page != ''){
				$albums_url = "https://graph.facebook.com/v2.1/".$page.'/albums?fields=id,link,count,from&limit=1000&access_token='.$token;
				$albums = json_decode($this->file_get_contents_curl($albums_url));
				// if there is album data
				if(!empty($albums->data)){
					$found = 0;
					...
					if($found > 0){
						$user_url = "https://graph.facebook.com/v2.1/".$page.'?fields=name,picture&access_token='.$token;
					...
					$main_url = "https://graph.facebook.com/v2.1/".$page.'?access_token='.$token;
						...
						if(isset($this->settings['fb_authed'])){
							...
							if($options_count == 1){
								...
								if($token != '' && strpos($token, '|')){
									$output = array('error' => sprintf(__("The page may be age or country restricted or contain alcohol related content. It is blocked for %1$s, try again by having another user profile added first. ", 'jig_td'), $user_name));
								}else{
		...
		function jig_verify_fb_authed(){
			...
			$albums_url = "https://graph.facebook.com/v2.1/".$user_id.'/albums?fields=id,link,count,from&limit=1000&access_token='.$token;
			 $albums = json_decode($this->file_get_contents_curl($albums_url));
			...
			if(!empty($albums->data)){
				...
				if($found > 0){
					$user_url = "https://graph.facebook.com/v2.1/".$user_id.'?fields=name,picture&access_token='.$token;
					$user = json_decode($this->file_get_contents_curl($user_url));
					$output = array(	
						'user_name' => $user->name,
						'picture' => ($this->settings['timthumb_path'] ?
				...
				if(!isset($albums->error->message)){
					...
					$output = array('error' => __('Access denied. In case of expired access, please re-authenticate. The error from Facebook', 'jig_td').': '.$albums->error->message);
				}
			}
			echo json_encode($output);
			die();
		...
		function jig_get_fb_albums(){
			...
			$albums_url = "https://graph.facebook.com/v2.1/".$user_id.'?fields=albums.limit(1000).fields(id,cover_photo,link,count,name,description,type,photos.limit(1).fields(images))&access_token='.$token;
		...
		function facebook_api_call($endpoint_url = '', $facebook_caching = 0, $limit = '', $photos_count = 0){	
			...
			if(!empty($results->albums->data)){
				$results->data = $results->albums->data;
			}
			// For single result, smart deeplinking
		...
		function jig_ng_image_search($query,$options,$order_by = 'sortorder', $order_dir = 'ASC',$limit){
			...
			if(in_array('tag', $options)){
				foreach ($query as $query_value) {
					$image_ids = $wpdb->get_col($wpdb->prepare("SELECT tr.object_id FROM $wpdb->terms as t
						...
						AND t.name LIKE %s
						{$limit_by}", '%'.str_replace('\_','_',like_escape(esc_sql($query_value))).'%', $limit));
					if(!empty($image_ids)){
			...
			if(!empty($option_like)){
				...
				foreach ($query as $query_value) {
					$where_clause = str_replace('%s', '%'.str_replace('\_','_',like_escape(esc_sql($query_value))).'%', implode(' OR ', $option_like));
